...
class Ship:
    def __init__(self, ship_s_starting_point: Dot, length: int, vertical: bool):
        self.ship_s_starting_point = ship_s_starting_point
        self.length = length
    ...
    def dots(self):
        ...
        for i in range(self.length):
            curr_x = self.ship_s_starting_point.x
            curr_y = self.ship_s_starting_point.y
            if self.vertical:
                curr_y += i
            else:
                curr_x += i
...
class Board:
    def __init__(self, size: int, hid=False):
        ...
        self.busy = []  # список занятых координат доски
        self.ships = []  # список точек размещенных кораблей
    ...
    def contour(self, ship, visible=False):
        ...
        for dot in ship.dots:
            for d_x, d_y in around:
                curr_dot = Dot(dot.x + d_x, dot.y + d_y)
...
class Game:
    ...
    def print_boards(self):  # вывод двух досок рядом по горизонтали
        ...
        print()
        print(
            "Ваша доска:".ljust((self.size + 1) * 4 - 1)
            + " " * self.size
            + "Доска компьютера:"
        )
        for s1, s2 in zip(
            self.pl.board.__str__().split("\n"), self.ai.board.__str__().split("\n")
        ):
